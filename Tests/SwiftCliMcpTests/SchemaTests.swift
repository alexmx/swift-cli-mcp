...
struct SchemaTests {
    ...
    // MARK: - @InputProperty Tests

    @Test("InputProperty encodes/decodes transparently")
    func propertyDescriptionCodable() throws {
        struct TestArgs: Codable {
            @InputProperty("A message")
            var message: String = ""
        }
    ...
    @Test("MCPToolInput extracts descriptions from @InputProperty")
    ...
    func toolInputMultipleTypes() {
        struct SearchArgs: MCPToolInput {
            @InputProperty("Search query")
            var query: String

            @InputProperty("Maximum results")
            var limit: Int

            @InputProperty("Include details")
            var verbose: Bool

            @InputProperty("Score threshold")
    ...
    @Test("MCPToolInput with optional @InputProperty")
    func toolInputOptionalProperty() {
        struct Args: MCPToolInput {
            @InputProperty("Required field")
            var name: String

            @InputProperty("Optional field")
            var tag: String?
        }
